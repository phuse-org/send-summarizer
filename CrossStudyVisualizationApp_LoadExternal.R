  ...
  dashboardBody(
    ...
    tabsetPanel(
      tabPanel("Data Source", 
              ...
              conditionalPanel(condition = "input.DataSourceChoice == 'BioCelerate' ",
                               h3("Example of 4 Studies (2 Dog, 2 Rat) for same target")
              ),
              column(width = 5, offset = 5, conditionalPanel(condition = "input.DataSourceChoice == 'BioCelerate'",
...
server <- shinyServer(function(input, output, session) {
   ...
   observeEvent(input$PLOT,{
     ...
     #Options for dataSource to be specified by user: BioCelerate, Local, and Database
     if (dataSource == "BioCelerate"){
       dataPaths <- read.ini('dataPaths - Template.ini')
       paths <- as.character(unlist(dataPaths[[dataSource]]))
       numstudies <- length(input$studies)
     } else if (dataSource == "Local"){
       if (is.list(input$folder) == FALSE){
         #Error Catch for if user did not specify local path
         showNotification("Please Select Local File Folder Path", type = "error")
         return() 
       }
        #Load .xpt data based on number of inputs
       numstudies <- length(input$LocalStudies)
       studypath <- paste(unlist(input$folder$path), collapse = '/')
      #Asign Name for number of studies loaded
       for (j in 1:numstudies){
         Name <- paste0('SENDStudy',as.character(j))
         print(paste0(Name, " = ", input$LocalStudies[j]))
         assign(Name,load.xpt.files(path = paste(homePath, studypath,"/",input$LocalStudies[j], sep = "")))
       }
       ...
       for (nj in 1:numstudies){
         ...
         #Create Error if there are more than 4 non-recovery dose groups
         if (length(StudyARMCDs) != 4){
           showNotification(paste0(input$LocalStudies[nj], " has more than 4 Non-Recovery Dose Arms"), type = "error")
           return()
         }
       }
