...
groupSEND <- function(dataset,targetDomain,dmFields=c('SEX','ARMCD','SETCD','USUBJID', 'RFSTDTC'),
  ...
  for (set in SETCD) {
    ...
    if ('TCNTRL' %in% dataset$tx$TXPARMCD) {
      if (is.na(as.numeric(TRTDOS[count])) == TRUE){
        TCNTRL[count] <- NA
      } else if (as.numeric(TRTDOS[count])==0) {
  ...
  groupedData <- cbind(groupedData[,(colnames(groupedData) %in% columns2move)],groupedData[,columnsNOmove])
