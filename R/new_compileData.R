...
new_compileData <- function(studyid, 
                             path_db,
                             xpt_files = FALSE,
                             fake_study = FALSE) {
browser()
  ...
  calculate_CompileData <- function(bw, dm, ds, ts, tx, pooldef, pp ) {
 ...
 # Function to read .xpt files 
  if (xpt_files == TRUE & fake_study == FALSE ) {
    ...
    master_CompileData <- calculate_CompileData(bw, dm, ds, ts, tx, pooldef, pp)
  ...
  #   pp <- haven::read_xpt(fs::path(xpt_dir,'pp.xpt'))
  #   
  # } else if (xpt_files == TRUE & fake_study == TRUE ){
  #   dm <- haven::read_xpt(fs::path(xpt_dir,'dm.xpt'))
  #   data.table::setDT(dm)
  #   ts <- haven::read_xpt(fs::path(xpt_dir,'ts.xpt'))
  #   data.table::setDT(ts)
